                    ...
                    <ul>
                        ...
                        <li class="product_skill_item" v-for="item in homeDatas">
                            <router-link :to="'/detail/'+item.productId" class="product_skill_item_link">
                                <img v-lazy="item.smallImageUrl" alt="" class="product_skill_item_cion lazy-img-fadein">
                                <p class="nowprice">
                                    <i style="text-decoration: none;font-style: normal;font-size: 12px">¥</i>
                                    <span class="now_price">{{item.priceResult.price}}</span>
                                </p>
                                <p class="oldprice">
                                    <i style="text-decoration: none;font-style: normal;font-size: 12px">¥</i>
                                    <del class="old_price">{{item.priceResult.listPrice}}</del>
                                </p>
                            </router-link>
                        </li>
  ...
  export default {
    ...
    methods: {
      getData() {
        let _this = this;
        //获取秒杀
        _this.$http.post('productSecKillList', qs.stringify({
          storeId: 10000
        })).then((res) => {
          if(res.data.productIds){
            _this.$http.post('productsSummary',qs.stringify({productIds:res.data.productIds.toString(),webSiteId:10000,prodCatalogId:10000,productStoreId:10000})).then((res)=>{
                console.log(res)
                _this.homeDatas = res.data.resultData;
            },(err)=>{
                console.log(err)
            })
          }
